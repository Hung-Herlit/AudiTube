...
local Highlight =
	loadstring(
		game:HttpGet("https://raw.githubusercontent.com/Hung-Herlit/AudiTube/refs/heads/main/highlight.lua")
	)()
...
-- Ghi file script mỗi khi remote được gọi vào thư mục "RemoteLogs/"
local function saveRemoteToFile(name, args, genScript)
    local function sanitize(str)
        return tostring(str):gsub("[^%w]", "_"):sub(1, 20)
    end

    local filenameParts = { sanitize(name) }
    for i, v in ipairs(args) do
        table.insert(filenameParts, sanitize(v))
    end

    local baseName = table.concat(filenameParts, "_")
    local i = 1

    local folder = "RemoteLogs"
    if makefolder and not isfolder(folder) then
        makefolder(folder)
    end

    local filename = folder .. "/" .. baseName .. "_" .. i .. ".lua"

    -- Đảm bảo không trùng tên file
    while isfile and isfile(filename) do
        i += 1
        filename = folder .. "/" .. baseName .. "_" .. i .. ".lua"
    end

    -- Ghi file nếu có quyền
    if writefile then
        writefile(filename, genScript)
    else
        warn("writefile() không khả dụng trên môi trường này.")
    end
end
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
    ...
    if blocked then
        log.GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY SIMPLESPY\n\n" .. log.GenScript
    end
    saveRemoteToFile(name, args, log.GenScript)
		if blocked then
